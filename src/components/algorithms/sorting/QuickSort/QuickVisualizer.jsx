...
export default function QuickVisualizer({
  ...
  // Build active call frames (quickSort recursion) without using in-place reverse()
  ...
  for (
    ...
    if (
      ...
      // find matching frame from top of stack
      for (let k = activeCallFrames.length - 1; k >= 0; k--) {
        const f = activeCallFrames[k];
        if (f.low === st.low && f.high === st.high) {
          f.mid = st.mid;
          break;
        }
      }
    ...
    if (
      ...
      for (let k = activeCallFrames.length - 1; k >= 0; k--) {
        const f = activeCallFrames[k];
        if (f.low === st.low && f.high === st.high) {
          f.pIndex = st.pIndex ?? st.pivotIndex;
          break;
        }
      }
      continue;
    ...
    if (st.phase === "left-complete") {
      for (let k = activeCallFrames.length - 1; k >= 0; k--) {
        if (activeCallFrames[k].kind === "call-left") {
          activeCallFrames.splice(k, 1);
          break;
        }
      }
      continue;
    ...
    if (st.phase === "right-complete") {
      for (let k = activeCallFrames.length - 1; k >= 0; k--) {
        if (activeCallFrames[k].kind === "call-right") {
          activeCallFrames.splice(k, 1);
          break;
        }
      }
    ...
    // If a condition-check step indicates base-case (low >= high), remove matching frame
    if (st.phase === "condition-check" && st.low !== undefined && st.high !== undefined) {
      if (st.low >= st.high) {
        // remove the most recent matching frame
        for (let k = activeCallFrames.length - 1; k >= 0; k--) {
          const f = activeCallFrames[k];
          if (f.low === st.low && f.high === st.high) {
            activeCallFrames.splice(k, 1);
  ...
  const currentFrame =
    activeCallFrames.length > 0
      ? activeCallFrames[activeCallFrames.length - 1]
      : null;

  // Build a map of pIndex/pivotIndex for each frame key (low-high) by scanning past steps once.
  // This avoids repeatedly scanning for each frame and prevents parent-frame pIndex from
  // disappearing when nested calls are emitted.
  const pIndexMap = new Map();
  ...
  for (
    ...
    if (!st) continue;
    if (st.low === undefined || st.high === undefined) continue;
    const key = `${st.low}-${st.high}`;
    if (st.pIndex !== undefined && st.pIndex !== null) {
      pIndexMap.set(key, st.pIndex);
      continue;
    }
    if (st.pivotIndex !== undefined && st.pivotIndex !== null) {
      pIndexMap.set(key, st.pivotIndex);
      continue;
    }
    if (finalPhases.has(st.phase) && st.j !== undefined && st.j !== null) {
      pIndexMap.set(key, st.j);
      continue;
    }
  }
  ...
  if (currentFrame) {
    for (
      ...
      if (st.randomIndex !== undefined && st.randomIndex !== null) {
        if (
          (st.low !== undefined &&
            st.high !== undefined &&
            st.low === currentFrame.low &&
            st.high === currentFrame.high) ||
          s === currentStepIndex
        ) {
    ...
    if (
      scopedRandomIndex == null &&
      currentStep?.randomIndex !== undefined &&
      currentStep?.randomIndex !== null
    ) {
  ...
  if (currentFrame) {
    ...
    if (
      scopedPivotValue == null &&
      currentStep?.pivotValue !== undefined &&
      currentStep?.pivotValue !== null
    ) {
  ...
  if (currentFrame) {
    for (
      ...
      if ((matchesFrame || isCurrent) && st.i !== undefined && st.i !== null)
        scopedI = st.i;
      if ((matchesFrame || isCurrent) && st.j !== undefined && st.j !== null)
        scopedJ = st.j;
    }
    ...
    if (
      ...
      scopedI = currentStep.i;
    if (
      scopedJ == null &&
      currentStep?.j !== undefined &&
      currentStep?.j !== null
    )
      scopedJ = currentStep.j;
  }
  ...
  try {
    if (currentStep?.phase === "partition-entry" || scopedRandomIndex != null) {
      console.debug(
        ...
        currentStep?.phase,
        "cur.randomIndex=",
        currentStep?.randomIndex,
        "scopedRandomIndex=",
        scopedRandomIndex
      );
      const nearby = sortingSteps
        .slice(
          Math.max(0, currentStepIndex - 3),
          Math.min(sortingSteps.length, currentStepIndex + 6)
        )
        .map((s, i) => ({
          i: i + Math.max(0, currentStepIndex - 3),
          phase: s?.phase,
          randomIndex: s?.randomIndex,
        }));
  ...
  const shouldShowRandomIndex =
    currentFrame &&
    PARTITION_PHASES.has(currentStep?.phase) &&
    scopedRandomIndex != null;
  const randomIndexObj = shouldShowRandomIndex
    ? { value: scopedRandomIndex }
    : null;

  const shouldShowPivotValue =
    currentFrame &&
    PARTITION_PHASES.has(currentStep?.phase) &&
    scopedPivotValue != null;
  const pivotValueObj = shouldShowPivotValue
    ? { value: scopedPivotValue }
    : null;
  const shouldShowI =
    currentFrame && PARTITION_PHASES.has(currentStep?.phase) && scopedI != null;
  const shouldShowJ =
    currentFrame && PARTITION_PHASES.has(currentStep?.phase) && scopedJ != null;
  const iObj = shouldShowI ? { value: scopedI } : null;
  const jObj = shouldShowJ ? { value: scopedJ } : null;

  if (!showCallUI && !randomIndexObj && !pivotValueObj && !iObj && !jObj)
    return null;

  return (
    <div className="mt-3 w-full px-8">
      {/* Row 1: call frames with per-frame pIndex */}
              ...
              >
                ...
                >
                  ...
                  <div className="text-sm font-mono truncate">{`low=${safeValue(
                    ...
                    {(() => {
                      // prefer explicit frame.pIndex, then pIndexMap value for this frame, then currentStep
                      const key = `${frame.low}-${frame.high}`;
                      const persisted = frame.pIndex ?? pIndexMap.get(key);
                      const showPlaceholder =
                        PARTITION_PHASES.has(currentStep?.phase) &&
                        currentFrame &&
                        idx === activeCallFrames.length - 1;
                      if (
                        persisted !== undefined ||
      ...
      {/* Row 2: randomIndex, pivot, i, j (centered) */}
      <div className="mt-4 flex items-center justify-center gap-4">
        ...
        {pivotValueObj && (
          <div className="min-h-10 w-36 py-1 rounded-lg flex items-center justify-center font-medium bg-sky-300 text-gray-900 shadow-md">
            <div className="text-center">
              <div className="text-xs text-gray-700">pivot</div>
              <div className="text-lg font-bold">
                {safeValue(pivotValueObj.value)}
              </div>
            </div>
          </div>
        )}
        {iObj && (
          <div className="min-h-10 w-28 py-1 rounded-lg flex items-center justify-center font-medium bg-lime-300 text-gray-900 shadow-md">
            <div className="text-center">
...
export function getQuickOverlay({
  sortingSteps = [],
  currentStepIndex = 0,
  currentStep = {},
} = {}) {
  ...
  for (
    ...
    const st = sortingSteps[i];
    if (!st?.phase) continue;
    if (["start"].includes(st.phase)) activeCallFrames.push(st);
    else if (
      ["base", "subarray-sorted"].includes(st.phase) &&
      activeCallFrames.length > 0
    )
